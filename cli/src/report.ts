...
export function renderReport(report: AuditReport, title: string): string {
  const lines: string[] = [];
  ...
  for (const check of report.checks) {
    ...
    if (!check.passed && check.fix) {
      ...
      if (check.fix.file) lines.push(`      at: ${check.fix.file}${check.fix.line ? `:${check.fix.line}` : ""}`);
    }
  }
  return lines.join("\n");
}
 ...
 */
export function renderBeforeAfter(before: AuditReport, after: AuditReport): string {
  const lines: string[] = [];
