...
export function envLocalIsGitignored(scope: ProjectScope): boolean {
  if (!scope.exists(".gitignore")) return false;
 ...
 */
export function writeAgentKeyToEnvLocal(scope: ProjectScope, key: string, value: string): void {
  if (!envLocalIsGitignored(scope)) {
    throw new UnsafeSecretWriteError(
      `Refusing to write ${key} to .env.local: this project's .gitignore does not cover .env.local, so writing the key here risks committing it to source control. Add ".env.local" (or ".env*") to your .gitignore and run this again.`,
    );
  }
