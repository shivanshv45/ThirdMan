function readPackageJson(scope) {
    if (!scope.exists("package.json"))
        return null;
    try {
...
const DETECTORS = [
    ...
    },
    {
        ...
        detect: (scope) => {
            ...
            for (const f of ["remix.config.js", "vite.config.ts"]) {
                if (scope.exists(f) && hasDependency(pkg, "@remix-run/dev"))
                    evidence.push(f);
            }
            return evidence;
        },
    ...
    },
    {
        ...
        detect: (scope) => {
            const evidence = [];
            const pkg = readPackageJson(scope);
            if (hasDependency(pkg, "@shopify/hydrogen"))
                evidence.push("package.json (@shopify/hydrogen dependency)");
            return evidence;
        },
    },
    {
 ...
 */
export function detectStack(scope) {
    const matches = [];
    for (const detector of DETECTORS) {
        const evidence = detector.detect(scope);
        if (evidence.length > 0)
            matches.push({ stack: detector.stack, evidence });
    }
    if (matches.length === 0) {
